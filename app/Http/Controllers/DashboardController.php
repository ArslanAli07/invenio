...
class DashboardController extends Controller
{
    public function index(): Response
    {
        ...
        // Low-stock products & variants: those where total ledger qty < reorder_level
        // Scan at the variant level if variants exist, otherwise product level
        $allProducts = Product::with('variants')->where('is_active', true)
        ...
        usort($lowStockItems, fn($a, $b) => $b['deficit'] <=> $a['deficit']);

        $lowStockCount = count($lowStockItems);
        // Cap to 8 for display
        $lowStockProducts = array_slice($lowStockItems, 0, 8);

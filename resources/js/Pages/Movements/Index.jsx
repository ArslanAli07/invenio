...
function TypeBadge({ type }) {
    ...
    const Icon = cfg.icon;
    return (
        <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-md text-xs font-medium border ${cfg.badge}`}>
...
function RefCell({ movement }) {
    ...
    if (isPo && movement.reference_id) {
        return (
            <Link
                ...
                className="inline-flex items-center gap-1.5 font-mono text-[10px] font-bold text-zinc-600 dark:text-zinc-400 bg-zinc-100 dark:bg-zinc-700 px-2 py-0.5 rounded-md border border-zinc-200 dark:border-zinc-600 hover:bg-blue-100 dark:hover:bg-blue-500/20 transition-colors"
            >
    ...
    return (
        <span className="inline-flex items-center gap-1.5 font-mono text-[10px] text-slate-600 dark:text-zinc-400 bg-slate-100 dark:bg-zinc-700 px-2 py-0.5 rounded border border-zinc-200 dark:border-zinc-700">
...
function StatChip({ label, value, color }) {
    const colors = {
        slate:   'bg-slate-100 dark:bg-zinc-700 text-zinc-700 dark:text-zinc-200 border-zinc-200 dark:border-zinc-700',
...
export default function Index({ movements, locations, filters = {}, summary }) {
    ...
    return (
        <AuthenticatedLayout>
            ...
            {/* ── Filter bar ────────────────────────────────────────────────── */}
            <div className="bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm p-4 mb-4">
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <form onSubmit={handleSearch} className="flex items-center gap-2 flex-1 min-w-[200px]">
                        <div className="relative flex-1">
                            ...
                            <input
                                ...
                                className="w-full pl-9 pr-4 py-2 text-sm bg-slate-50 dark:bg-zinc-700 border border-zinc-200 dark:border-zinc-700 text-zinc-900 dark:text-zinc-100 placeholder-slate-400 dark:placeholder-ink-500 rounded-xl focus:outline-none focus:ring-2 focus:ring-zinc-400/30 focus:border-blue-500 transition"
                            />
                        </div>
                        <Button
                            type="submit"
                            size="sm"
                            className="rounded-xl bg-[#6b7c5c] hover:bg-[#5a6b4c] text-white font-semibold px-4 shrink-0"
                        >
                            Search
                        </Button>
                    </form>
                    ...
                    <select
                        ...
                        className="text-sm bg-slate-50 dark:bg-zinc-700 border border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-zinc-400/30 cursor-pointer"
                    >
                    ...
                    {hasFilters && (
                        <button
                            onClick={clearAll}
                            className="inline-flex items-center gap-1.5 text-sm text-zinc-500 dark:text-zinc-400 hover:text-rose-600 dark:hover:text-rose-400 font-semibold px-3 py-2 rounded-xl hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-colors"
                        >
            ...
            <div className="bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm overflow-hidden flex flex-col">
                ...
                {/* Table header meta */}
                <div className="px-6 py-3.5 border-b border-zinc-100 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-700 flex items-center justify-between">
                    <span className="text-xs text-zinc-500 dark:text-zinc-400 font-medium">
                        {movements.total > 0
                            ? <>Showing <span className="font-semibold text-zinc-700 dark:text-zinc-200">{movements.from}–{movements.to}</span> of <span className="font-semibold text-zinc-700 dark:text-zinc-200">{movements.total.toLocaleString()}</span> entries</>
                            : 'No entries match your filters'}
                    </span>
                    {hasFilters && (
                        <span className="text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-blue-50 dark:bg-blue-500/10 text-[#6b7c5c] dark:text-[#8fa67a] border border-blue-100 dark:border-blue-500/20">
                            Filtered
                        </span>
                    )}
                </div>
                        ...
                        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-700">
                            {movements.data.length > 0 ? (
                                movements.data.map((m) => {
                                    ...
                                    return (
                                        <tr key={m.id} className="hover:bg-slate-50/30 dark:hover:bg-zinc-700/40 transition-colors group">

                                            {/* Timestamp */}
                                            <td className="px-6 py-3.5 text-xs text-zinc-500 dark:text-zinc-400 font-medium whitespace-nowrap">
                                            ...
                                            <td className="px-6 py-3.5 text-right">
                                                ...
                                                <span className="block text-[10px] text-zinc-400 dark:text-zinc-500 font-semibold uppercase mt-0.5 text-right">
                                                    {m.product?.unit}
                                                </span>
                                            </td>
                                            ...
                                            <td className="px-6 py-3.5">
                                                ...
                                                {m.location?.code && (
                                                    <span className="font-mono text-[10px] text-zinc-400 dark:text-zinc-500 uppercase ml-5">
                                                        {m.location.code}
                                                    </span>
                                                )}
                                            </td>
                                            ...
                                            <td className="px-6 py-3.5">
                                                <span className="flex items-center gap-1.5 text-xs font-semibold text-slate-600 dark:text-zinc-400">
                                                    <div className="h-5 w-5 rounded-full from-[#1B4FD8] to-indigo-500 text-white font-bold flex items-center justify-center text-[9px] shrink-0 uppercase">
                                    ...
                                    <td colSpan="8" className="px-6 py-16 text-center">
                                        <div className="flex flex-col items-center gap-3 text-zinc-400 dark:text-zinc-500">
                                            ...
                                            {hasFilters && (
                                                <button
                                                    onClick={clearAll}
                                                    className="mt-1 text-xs text-[#6b7c5c] dark:text-[#8fa67a] font-semibold hover:underline"
                                                >
                                                    Clear all filters
                                                </button>
                                            )}
                ...
                {movements.links && movements.data.length > 0 && (
                    <div className="px-6 py-4 border-t border-zinc-200 dark:border-zinc-700 flex items-center justify-between bg-zinc-50 dark:bg-zinc-700 flex-wrap gap-3">
                        <span className="text-xs text-zinc-500 dark:text-zinc-400">
                            Page <span className="font-semibold text-slate-800 dark:text-zinc-200">{movements.current_page}</span> of{' '}
                            <span className="font-semibold text-slate-800 dark:text-zinc-200">{movements.last_page}</span>
                        </span>

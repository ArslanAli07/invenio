        ...
        <AuthenticatedLayout>
            ...
            <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">Manage vendor relations, contact info, and active supply partners.</p>
                </div>
                {can.create && (
                    <Button 
                        ...
                        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-700">
                            {suppliers.data.length > 0 ? (
                                suppliers.data.map((supplier) => (
                                    <tr key={supplier.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-700/50 transition-colors">
                                        <td className="px-6 py-4">
                                            <span className="font-semibold text-zinc-900 dark:text-zinc-100 text-sm">{supplier.name}</span>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className="text-xs text-zinc-500 dark:text-zinc-400 line-clamp-1 max-w-[200px]" title={supplier.address}>
                                                {supplier.address || '—'}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4">
                                ...
                                <tr>
                                    <td colSpan="5" className="px-6 py-16 text-center">
                                        <div className="flex flex-col items-center justify-center max-w-sm mx-auto">
                                            ...
                                            <h3 className="text-base font-bold text-zinc-900 dark:text-zinc-100">No suppliers found</h3>
                ...
                {suppliers.links && suppliers.data.length > 0 && (
                    <div className="px-6 py-4 border-t border-zinc-200 dark:border-zinc-700 flex items-center justify-between bg-zinc-50 dark:bg-zinc-700">
                        <span className="text-xs text-zinc-500 dark:text-zinc-400">
                            Showing <span className="font-semibold text-slate-800 dark:text-zinc-200">{suppliers.from}</span> to{' '}
                            <span className="font-semibold text-slate-800 dark:text-zinc-200">{suppliers.to}</span> of{' '}
                            <span className="font-semibold text-slate-800 dark:text-zinc-200">{suppliers.total}</span> entries
                        </span>
                                ...
                                if (label.includes('Previous')) {
                                    return (
                                        <Button
                                            ...
                                            variant="outline"
                                            size="sm"
                                            className="rounded-xl flex items-center gap-1.5 hover:bg-slate-100 dark:hover:bg-zinc-700 dark:border-zinc-600"
                                ...
                                return (
                                    <Button
                                        ...
                                        className={`rounded-xl h-9 w-9 p-0 ${
                                            link.active ? 'bg-[#6b7c5c] hover:bg-[#5a6b4c]' : 'hover:bg-zinc-100 dark:hover:bg-zinc-700 dark:border-zinc-600'
                                        }`}
                                        onClick={() => router.get(link.url, {}, { preserveState: true })}
                                    >
                                        {label}
                ...
                <SheetContent side="right" className="w-full sm:max-w-md bg-white dark:bg-zinc-800 p-6 border-l border-zinc-200 dark:border-zinc-700 shadow-2xl flex flex-col h-full overflow-hidden z-[210]">
                    ...
                    <form onSubmit={handleFormSubmit} className="flex-1 flex flex-col min-h-0">
                        <div className="space-y-4 overflow-y-auto pr-1 flex-1 mb-4 min-h-0">
                            <div>
                                <InputLabel htmlFor="name" value="Supplier / Company Name" className="text-zinc-700 dark:text-zinc-200 font-semibold mb-1.5" />
                                <TextInput
                                    id="name"
                                    type="text"
                                    name="name"
                            ...
                            <div>
                                <InputLabel htmlFor="phone" value="Phone Number" className="text-zinc-700 dark:text-zinc-200 font-semibold mb-1.5" />
                                <TextInput
                                    id="phone"
                                    type="text"
                                    name="phone"
                            ...
                            <div>
                                <InputLabel htmlFor="address" value="Company Address" className="text-zinc-700 dark:text-zinc-200 font-semibold mb-1.5" />
                                <textarea
                                    id="address"
                                    name="address"
                                    value={data.address}
                            ...
                            <div>
                                <InputLabel htmlFor="notes" value="Internal Procurement Notes" className="text-zinc-700 dark:text-zinc-200 font-semibold mb-1.5" />
                                <textarea
                                    id="notes"
                                    name="notes"
                                    value={data.notes}
